-- ============================================================
-- MIGRATION: Add indexes on tenant_id for performance
-- ============================================================
-- Improves query performance on multi-tenant tables
-- by indexing the most frequently filtered column

-- 1. Index on tenants table (id lookup)
CREATE INDEX IF NOT EXISTS idx_tenants_id ON public.tenants(id);

-- 2. Index on customers (tenant_id + filtering by tenant)
CREATE INDEX IF NOT EXISTS idx_customers_tenant_id ON public.customers(tenant_id);
CREATE INDEX IF NOT EXISTS idx_customers_tenant_ctv ON public.customers(tenant_id, ctv_id);

-- 3. Index on customer_crop_areas (tenant_id + customer_id)
...
-- 4. Index on it_se_configurations (tenant_id + common filters)
...
-- 5. Index on customer_faixas (tenant_id + customer_id)
...
-- 8. Index on setup_budgets (tenant_id + mes + id_ctv)
...
-- 9. Index on faturamento_snapshots (tenant_id + mes)
...
-- 10. Indexes on new config tables
...
CREATE INDEX IF NOT EXISTS idx_planejamento_cliente_segmento_tenant_id ON public.planejamento_cliente_segmento(tenant_id);
CREATE INDEX IF NOT EXISTS idx_planejamento_cliente_segmento_customer_id ON public.planejamento_cliente_segmento(customer_id);
